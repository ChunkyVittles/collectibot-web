...
export async function generateMetadata({ params }: Props) {
  ...
  const result = await pool.query<{ publisher: string }>(
    `SELECT DISTINCT publisher FROM postcards WHERE publisher IS NOT NULL`
  );
  const match = result.rows.find(r => slugify(r.publisher) === slug);
  if (!match) return { title: "Publisher Not Found" };
  return {
    title: `${match.publisher} Postcards - Collectibot`,
    description: `Browse vintage postcards published by ${match.publisher}. Scanned front and back with detailed metadata.`,
  };
}
...
export default async function PublisherPage({ params }: Props) {
  ...
  const pubResult = await pool.query<{ publisher: string }>(
    `SELECT DISTINCT publisher FROM postcards WHERE publisher IS NOT NULL`
  );
  const match = pubResult.rows.find(r => slugify(r.publisher) === slug);
  if (!match) return notFound();
  const publisherName = match.publisher;
  ...
  const result = await pool.query<{
    ...
    `SELECT id, description, era, postmark_city, postmark_state, postmark_year
     FROM postcards WHERE publisher = $1
  ...
  const eras = [...new Set(postcards.map(p => p.era).filter(Boolean))];
  ...
  return (
    <div style={{ maxWidth: 800, margin: "40px auto", padding: "0 20px", fontFamily: "system-ui" }}>
      ...
      <h1 style={{ marginTop: 16, marginBottom: 4 }}>{publisherName}</h1>
      <p style={{ color: "#666", margin: "0 0 8px 0" }}>
        {[yearRange, eras.length > 0 ? eras.join(", ") : null].filter(Boolean).join(" · ")}
      </p>
      <p style={{ color: "#999", margin: "0 0 16px 0" }}>
        {postcards.length} postcard{postcards.length !== 1 ? "s" : ""}
      </p>

      {/* Hero image — first postcard */}
      {postcards.length > 0 && (
...
function PostcardTable({ postcards, cacheBust }: { postcards: Array<{ id: number; description: string | null; era: string | null; postmark_city: string | null; postmark_state: string | null; postmark_year: number | null }>; cacheBust: number }) {
  return (
